...
pub fn handle_input(boomer_world: &mut BoomerWorld, world: &mut World) {
    ...
    let handles = boomer_world.resources.ui_handles.pause;
    ui_set_text(
        world,
        handles.difficulty_label,
        &format!(
            "DIFFICULTY: {}",
            boomer_world.resources.settings.difficulty.label()
        ),
    );
    ...
    if clicked_difficulty {
        let next = boomer_world.resources.settings.difficulty.next();
        boomer_world.resources.settings.difficulty = next;
